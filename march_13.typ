...
*Lagrange's Theorm*: For group G with n elements the order of $g$ divides $n$ (written $g|n$) and $g$ divide $n$ means that n is a multiple of $g$, so there exists  $K in ZZ "with" gk = n$.
Note: $ZZ_m^* = { a in ZZ_m : gcd(qm) = 1}$ is a group since $a^-1 "mod" m$ exists when $a$ and $m$ are coprime and $a^-1$ is found by $EEA(a, m)$. 
...
So $(p-1)/d$ has some prime divison, say $q | (p-1)/d$. Rewriting this, we have $E chi in ZZ$ $ qk = (p-1)/d$ which implies $dk = (p-1)/q$. Since $alpha^d eq.triple 1 ("mod" p)$, raising this to the power $k$, we get $alpha^(dk) = alpha^((p-1)/q) eq.triple 1 ("mod" p)$, a contradiction. box

*The RSA Cryptosystem*: Let $n=pq$ where $p,q$ are distinct primes. Let $epsilon = p = ZZ_n$. In practice  
